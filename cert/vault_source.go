...
func (s *VaultSource) setToken(c *api.Client) error {
	...
	logging.Info("[INFO] vault: Unwrapped token %s", s.vaultToken)
...
func (s *VaultSource) load(path string) (pemBlocks map[string][]byte, err error) {
	...
	get := func(name, typ string, secret *api.Secret) {
		...
		case []byte:
			b = v.([]byte)
		default:
			logging.Warn("[WARN] cert: key %s has type %T", name, v)
			return
		}
	...
	if err != nil {
		...
		logging.Warn("[WARN] vault: Failed to renew token. %s", err)
	}
	...
	for _, s := range certs.Data["keys"].([]interface{}) {
		...
		if err != nil {
			logging.Warn("[WARN] cert: Failed to read %s from Vault: %s", p, err)
			continue
		}

...
func (s ConsulSource) Certificates() chan []tls.Certificate {
	...
	if err != nil {
		logging.Error("[ERROR] cert: Failed to parse consul url. %s", err)
	}

	client, err := api.NewClient(config)
	if err != nil {
		logging.Error("[ERROR] cert: Failed to create consul client. %s", err)
	}
	...
	go func() {
		for pemBlocks := range pemBlocksCh {
			...
			if err != nil {
				logging.Error("[ERROR] cert: Failed to load certificates. %s", err)
				continue
			}
			ch <- certs
		}
...
func watchKV(client *api.Client, key string, pemBlocks chan map[string][]byte) {
	...
	for {
		...
		if err != nil {
			logging.Warn("[WARN] cert: Error fetching certificates from %s. %v", key, err)
			time.Sleep(time.Second)
			continue
		}

		if !reflect.DeepEqual(value, lastValue) || index != lastIndex {
			logging.Info("[INFO] cert: Certificate index changed to #%d", index)
			pemBlocks <- value

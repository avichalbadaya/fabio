...
func (p *tcpSNIProxy) Serve(in net.Conn) {
	...
	if !ok {
		...
		logging.Debug("[DEBUG] tcp+sni: TLS handshake failed")
		return
	}

	if serverName == "" {
		fmt.Fprintln(in, "server_name missing")
		logging.Debug("[DEBUG] tcp+sni: server_name missing")
		return
	}

	t := route.GetTable().LookupHost(serverName)
	if t == nil {
		logging.Warn("[WARN] tcp+sni: No route for ", serverName)
		return
	}

	out, err := net.DialTimeout("tcp", t.URL.Host, p.cfg.DialTimeout)
	if err != nil {
		logging.Warn("[WARN] tcp+sni: cannot connect to upstream ", t.URL.Host)
		return
	}
	defer out.Close()
	...
	if err != nil {
		logging.Warn("[WARN] tcp+sni: copy client hello failed. ", err)
		return
	}
